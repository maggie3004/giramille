...
@app.command()
def main(
    input_dir: Path = typer.Option(..., exists=True, help="Input directory with images"),
    out_dir: Path = typer.Option(Path("outputs/vectors"), help="Output directory for vectors"),
    image_size: int = typer.Option(256, help="Image size for processing"),
    num_classes: int = typer.Option(16, help="Number of classes"),
    max_layers: int = typer.Option(20, help="Maximum layers"),
    max_anchors: int = typer.Option(300, help="Maximum anchors"),
    weights: Path = typer.Option(..., exists=True, help="Path to trained SmallUNet .pth weights"),
):
