...
export default function HomePage() {
  ...
  // Dataset images mapping for Giramille style (single definition)
  const datasetImages = {
    // Animals
    ...
    'conditioner': ['conditionador.png'],
    'wet wipes': ['lenço umedecido.png'],
    'hand sanitizer': ['gel antisseptico.png']
  };

  const BACKEND = typeof window !== "undefined" ? (process.env.NEXT_PUBLIC_API_BASE || "http://127.0.0.1:5000") : "http://127.0.0.1:5000";

  // utility to normalise backend history entries into historyImgs URLs
  async function refreshHistoryImgs(setHistoryImgs: (v: React.SetStateAction<string[]>) => void) {
    try {
      const r = await fetch(`${BACKEND}/api/history`);
      if (!r.ok) {
        console.warn("failed to fetch history", r.status);
        return;
      }
      ...
      const urls = items.map((it: any) => {
        ...
        if (it.image && typeof it.image === "string") {
          ...
          return it.image.startsWith("http") ? it.image : `${BACKEND}${it.image}`;
        }
        if (it.filename) {
          return `${BACKEND}/storage/${it.filename}`;
        }
        return null;
      ...
      console.warn("refreshHistoryImgs error", e);
    }
  }

  useEffect(() => {
    refreshHistoryImgs(setHistoryImgs);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // helpers: check/poll an URL until available
  ...
  async function waitForUrl(url: string, timeoutMs = 60000, intervalMs = 1000): Promise<boolean> {
  ...
  const generateImageFromPrompt = async (promptText: string, type: 'png' | 'vector'): Promise<string> => {
    ...
    try {
      const response = await fetch('/api/generate', {
        ...
        body: JSON.stringify({
          prompt: `${promptText}, Giramille style, professional illustration`,
          style: type,
          quality: 'balanced',
          width: 512,
          height: 512
  ...
  const handleGenVector = async () => {
    ...
    try {
      ...
      alert('Error generating image: ' + (error.message || error));
    }
  };
  ...
  // add this helper inside the HomePage component (place it before the "Render UI (unchanged)" return)
  async function generateImage(promptText: string) {
    // use BACKEND constant so requests go to the backend server (or NEXT_PUBLIC_API_BASE)
    const url = `${BACKEND}/api/generate`;
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ prompt: promptText, width: 512, height: 512 }),
    });
    const j = await res.json();
    if (j.success && typeof j.image === "string") {
      // Normalize returned path: if backend returns "/storage/xxx" prefix with BACKEND
      const img: string = j.image;
      if (img.startsWith("/storage")) return `${BACKEND}${img}`;
      return img;
    }
