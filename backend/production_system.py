    ...
    def generate_image(self, prompt: str, style: str = "png", quality: str = "balanced") -> Dict:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                logger.info(f"✅ Generated image in {generation_time:.2f}s")
...
if __name__ == "__main__":
    ...
    for prompt in test_prompts:
        ...
        if result['success']:
            logger.info(f"✅ Generated: {prompt} in {result['generation_time']:.2f}s")
        else:
            logger.error(f"❌ Failed: {prompt} - {result['error']}")
